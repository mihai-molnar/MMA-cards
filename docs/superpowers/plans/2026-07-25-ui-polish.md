...
Note: `BattleHud.RESULT_PANEL_Z` is added in Task 4. Until then this suite's final two checks fail to resolve. Add the constant to `battle_hud.gd` **now**, as a one-line change, so this task is self-contained:

```gdscript
const RESULT_PANEL_Z: int = 100
```

Place it with the other constants at the top of `battle_hud.gd`. Task 4 uses it.
